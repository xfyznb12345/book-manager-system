<template>
	<view>
		<view class="image-list" v-for="(imgdata,imgdataKey) in imglistthree" :key="imgdataKey">
			<!-- 标题 -->
			...
			<!-- 图文列表 -->
			<view class="books" v-for="(information,dataKey) in imgdata.data" :key="dataKey" @click="imageTextBtn(information)">
				<view class="books-img">
					<image :src="information.url || url[dataKey]" />
				</view>
				<view class="books-text">
					<p class="head">{{information.title}}</p>
					<view class="title-box">
						<p class="title">{{ information.author || '不祥'}}</p>
						<!-- <p class="title text-right">￥{{information.price}}</p> -->
					</view>
					<view class="title-bottom">
						<p class="title">{{information.abstract || '暂无简介'}}</p>
	...
	export default {
		...
		methods: {
			imageTextBtn(imageTextBtn) {
				...
				uni.navigateTo({
					url: `../../pages/booksInfo/booksInfo?id=${imageTextBtn._id}`
				})
			},
			scrollToLower() {
				console.log('====')
